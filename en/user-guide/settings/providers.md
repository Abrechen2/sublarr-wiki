...
Some scraper-based providers (primarily Kitsunekko and Gestdown under heavy load) may serve CAPTCHA challenges that block automated searches. Sublarr supports routing these through an external solving service.

| Setting | Env Variable | Description |
|---------|--------------|-------------|
| Provider | `SUBLARR_ANTI_CAPTCHA_PROVIDER` | Service to use: `2captcha`, `anticaptcha`, or `capsolver`. Leave empty to disable. |
| API Key | `SUBLARR_ANTI_CAPTCHA_API_KEY` | API key for the selected service. Only required when a provider is set. |

> Anti-captcha is only invoked when a provider returns a CAPTCHA challenge — it does not affect normal requests. For most homelab setups it is not required.
